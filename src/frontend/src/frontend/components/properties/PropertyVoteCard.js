...
const PropertyVoteCard = ({ property, onVoteChange }) => {
  ...
  useEffect(() => {
    const fetchVoteData = async () => {
      try {
        ...
        const voteOptionsResponse = await axios.get(`/api/properties/${property.id}/vote-options`);
        let options = voteOptionsResponse.data.voteOptions;
        
        // Get current vote counts
        const voteCountsResponse = await axios.get(`/api/properties/${property.id}/votes`);
        ...
        if (isAuthenticated && userData) {
          const userVoteResponse = await axios.get(`/api/properties/${property.id}/votes/user`);
  ...
  const handleVoteSubmit = async () => {
    ...
    try {
      ...
      if (userVote) {
        await axios.put(`/api/votes/${userVote.id}`, voteData);
      } else {
