...
const EditPropertyPage = () => {
  ...
  useEffect(() => {
    const fetchProperty = async () => {
      try {
        ...
        if (property.user_id !== userData?.id) {
          navigate('/properties');
          return;
        }
        ...
        setFormData({
          ...
          current_worth: property.current_worth || '',
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        const response = await axios.get('/api/properties/categories');
  ...
  const handleNewImages = (files) => {
    const newImages = Array.from(files).map((file) => {
      ...
      const isPrimary = imagePreviewUrls.length === 0 && existingImages.length === 0;
  ...
  // Set image as primary (for new images)
  ...
  // Set image as primary (for existing images)
  const setExistingImageAsPrimary = (index) => {
    // Remove primary from new images
    setImagePreviewUrls(prev => prev.map(img => ({
      ...img,
      isPrimary: false
    })));
  ...
  const removeNewImage = (index) => {
    setImagePreviewUrls(prev => {
      ...
      // If we removed the primary image and there are still new images left,
  ...
  const removeExistingImage = (index) => {
    ...
    setExistingImages(prev => {
      ...
      // If we removed the primary image and there are still existing images left,
      // set the first one as primary
      if (imageToRemove.isPrimary && newImages.length > 0) {
        newImages[0].isPrimary = true;
      } else if (imageToRemove.isPrimary && newImages.length === 0 && imagePreviewUrls.length > 0) {
        // If no existing images but we have new ones, set first new one as primary
        setImagePreviewUrls(prevNew => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error.response?.data?.message) {
        ...
        setErrors({ general: 'Error updating property. Please try again.' });
      }
    } finally {
      setLoading(false);
    }
  ...
  if (loadingProperty || loadingCategories) {
    return (
      <PageContainer>
        <Loader size="large" text="Loading property data..." />
      </PageContainer>
    );
  }
  ...
  return (
    <PageContainer>
      <BackLink to={`/properties/${id}`}>
        <FaArrowLeft /> Back to Property
      </BackLink>
      
      <PageHeader>
        <PageTitle>Edit Property</PageTitle>
        <PageSubtitle>Update your property listing information</PageSubtitle>
      </PageHeader>
              ...
              <ImageUploadContainer>
                ...
                {imagePreviewUrls.map((image, index) => (
                  <ImagePreview key={`new-${index}`}>
                    <img src={image.url} alt={`New property ${index + 1}`} />
                    {image.isPrimary && <PrimaryBadge>Primary</PrimaryBadge>}
                    <ImageControls>
                      {!image.isPrimary && (
                        <ImageControlButton 
